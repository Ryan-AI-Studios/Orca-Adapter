...
use crate::convert::{convert_archives, format_report_markdown, refuse_output_equals_input};
...
use crate::settings::{parse_project_settings, string_array_field, string_field};
...
fn convert_graft__output_project_settings__has_template_printer_model() {
    ...
    let report =
        convert_archives(&mut src, &mut tpl, out_buf, &SlotMap::identity(), true).expect("convert");
    assert_eq!(
    ...
    convert_archives(&mut src, &mut tpl, &mut out_buf, &SlotMap::identity(), true)
        .expect("convert2");
...
fn strip_list__removes_custom_gcode_and_gcode_members() {
    ...
    let mut tpl = open_mem(&template);
    let mut out_buf = Cursor::new(Vec::new());
    let report = convert_archives(&mut src, &mut tpl, &mut out_buf, &SlotMap::identity(), true)
        .expect("convert");
...
fn convert_archives__non_identity__model_settings_changed() {
    ...
    convert_archives(&mut src, &mut tpl, &mut out_buf, &map, true).expect("convert");
    let mut out_zip = open_mem(&out_buf.into_inner());
    let after = read_member_bytes(&mut out_zip, MODEL_SETTINGS).expect("after");
    assert_ne!(
        before, after,
...
fn report_markdown__contains_map_and_strip() {
    ...
    let mut out_buf = Cursor::new(Vec::new());
    let map = SlotMap::parse("1=2,2=1,3=3,4=4").expect("map");
    let report = convert_archives(&mut src, &mut tpl, &mut out_buf, &map, true).expect("convert");
...
fn convert_archives__identity_used_slot5__errors() {
    ...
    let mut tpl = open_mem(&template);
    let mut out_buf = Cursor::new(Vec::new());
    let err = convert_archives(&mut src, &mut tpl, &mut out_buf, &SlotMap::identity(), true)
